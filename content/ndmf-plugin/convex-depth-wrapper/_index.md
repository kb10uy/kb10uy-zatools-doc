...
ここまでの内容でも利用可能ですが、より高度な設定を行うこともできます。
...
### 個別の SkinnedMeshRenderer に生成する (>= 3.8.0)
...
### 凸包生成時に使用する BlendShape 値を上書きする (>= 3.9.0)

デフォルトではビルド時に適用されている SkinnedMeshRenderer の BlendShape 設定値を固定して凸包メッシュを生成しますが、一部の表情やまつ毛のハネなど、それらを無視したい場合に **上級者向け/BlendShapeオーバーライド** の値を設定することができます。
