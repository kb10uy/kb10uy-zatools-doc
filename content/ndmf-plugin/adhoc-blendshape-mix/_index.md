...
(>=3.2.0) パフォーマンス上の問題により、デフォルトで無効になっています。
...
* **JSON から読み込み**: JSON ファイルから設定を読み込み、このコンポーネントに反映します。
...
**JSON から設定を読み込む場合、存在しない BlendShape の名前が指定することができます。**
最終的に Ad-Hoc BlendShape Mix が処理されるタイミングまでにその名前のものが存在するならば、処理対象とすることができます。存在しなければ単に無視されます。
これは [Ad-Hoc BlendShape Split](../adhoc-blendshape-split/_index.md) と併用する際に有用です。
