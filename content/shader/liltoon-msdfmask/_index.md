...
**鋭さ** パラメーターはマスク境界のアンチエイリアスの鋭さを指定します。値が大きいほど境界が急峻になり、1 に設定すると完全に二値のみとるようになります(HLSL 的には step 関数と同等になります)。
描画領域に対して MSDF テクスチャの UV 密度が極めて低い場合、計算精度により境界が階段状のグラデーションになってしまうことがあります。その場合、値をより大きくしてください。
